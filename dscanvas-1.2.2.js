...
(function(win,fun){
    ...
    var $s =window.$s||{},config ={width:'100%',height:'100%',baseURI:'movie/',pre:'ds',canvas:'',file:'.json'};
    window.$s=$s;$s.config =config;$s.mains =[];
    $s.Stage = null;//多舞台情况，要重置此对象
...
//point 全局坐标
        ...
        if ($s.__activeType.indexOf(type) != -1) {
            ...
            if ($s.__mouseType.indexOf(type) != -1){
                ...
                fun = function(event){
                    ...
                    if (self.stage.focus) {
                        if(dsChildOf(stage.focus,dsInteractiveObject))self.stage.focus.__mouseaction(event);
                    }else{
    ...
    dsInteractiveObject.prototype.__beAddTo = function(displayObject){
...
//渲染副本内容
    ...
    dsStage.prototype.__init=function(canvas){
        ...
        function frame() {
            ...
            if(self.autorender)window.requestAnimationFrame(frame);
        }
        ...
        window.addEventListener(MouseEvent.RESIZE,function (e) {
            var parent = self.__canvas.parentNode;
            if(parent){
                if(typeof self._config.width =='string'){
                    var w = parseFloat(self._config.width)/100*parent.offsetWidth;
                    self.__canvas.width = w;
                }else{
                    self.__canvas.width = Math.min(self._config.width,parent.offsetWidth);
                }
                if(typeof self._config.height =='string'){
                    var h = parseFloat(self._config.height)/100*(parent.offsetHeight-4);
                    self.__canvas.height = h;
                }else{
                    self.__canvas.height = Math.min(self._config.height,parent.offsetHeight);
                }
            }
            self._stageWidth = self.__canvas.width;
            self._stageHeight = self.__canvas.height;
            self.invalidate();
            self.dispatchEvent($s.dsEvent(Event.RESIZE,false,false))
        })
    };
    ...
    dsStage.prototype.toHtmlElement = function(){
...
//此函数来源于 createjs.BitmapData;
    ...
    // dsTextField.prototype.__beAddTo = function(displayObject){
    //     this.__call(dsInteractiveObject,"__beAddTo", [displayObject]);
    //     var self = this;
    //     this.stage.addEventListener(MouseEvent.RESIZE,function () {
    //         self.text=self.text;
    //     })
    // }
    ...
    dsTextField.prototype.__render = function (ctx) {
        if(!this._text ||!this.visible)return
...
//行
    ...
    p.load = function(source){
        if(!source)return;
        var url = $s.Stage._config.baseURI+source+$s.Stage._config.file;
    ...
    p.__loadManifest = function(d){
        ...
        for (var i = 0; i < d.frames.length; i++) {
            var m = new Image();
            m.src = $s.Stage._config.baseURI+d.frames[i].filename + d.meta.image;
    ...
    p.__loadSequence=function(d){
        var self = this;
        var img = new Image();
        img.src = $s.Stage._config.baseURI + d.meta.image;
...
//图形加载
    ...
    dsLoader.prototype.__render = function (ctx) {
        if (this.data == null || !this.visible)return false;
